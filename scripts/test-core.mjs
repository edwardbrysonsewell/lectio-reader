...
// Whitaker's Words short definitions: most frequent reading first, irregulars and suffixes handled.
{const parses=read('whitaker/parses.json');const w=form=>(read('whitaker/forms/'+shardFor(form)+'.json')[form]||[]).map(([id,...ps])=>({e:read('whitaker/entries-'+Math.floor(id/4000)+'.json')[id],ps:ps.map(i=>parses[i])})).sort((a,b)=>a.e[4]-b.e[4]);
 for(const [form,head,parse] of [['est','sum, esse, fui, futurus','present active indicative, 3rd singular'],['amasti','amo, amare, amavi, amatus','perfect active indicative, 2nd singular'],['audierat','audio, audire, audivi, auditus','pluperfect active indicative, 3rd singular'],['conatur','conor, conari, conatus sum','present deponent indicative, 3rd singular'],['eandem','idem, eadem, idem','accusative singular feminine'],['cuiuscumque','quicumque, quaecumque, quodcumque','genitive singular'],['populique','populus, populi m.','genitive singular + -que'],['nobiscum','nos','ablative plural masculine/feminine + -cum']]){const r=w(form);assert(r.length,form+' has Whitaker readings');assert.equal(r[0].e[0],head,form);assert(r[0].ps.includes(parse),form+': '+r[0].ps.join(' | '));}}
